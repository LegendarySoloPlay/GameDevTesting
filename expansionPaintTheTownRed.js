...
async function invadeTheDailyBugleNewsHQTwist() {
    ...
    return new Promise((resolve) => {
        ...
        instructionsElement.innerHTML = 'SELECT A HERO IN THE HQ TO KO. THEY WILL BE REPLACED BY THE HIGHEST-<img src="Visual Assets/Icons/Attack.svg" alt="Attack Icon" class="console-card-icons"> VILLAIN IN THE CITY.';
...
async function dailyBugleVillainToHQ(selectedHQIndex) {
    ...
    const maxAttack = Math.max(...villainsInCity.map(villain => recalculateVillainAttack(villain)));

    // Filter villains with the highest recalculated attack
    const highestAttackVillains = villainsInCity.filter(villain => 
        recalculateVillainAttack(villain) === maxAttack
    );
    ...
    if (highestAttackVillains.length === 1) {
        ...
        onscreenConsole.log(`<span class="console-highlights">${villainToRemove.name}</span> is the highest-<img src="Visual Assets/Icons/Attack.svg" alt="Attack Icon" class="console-card-icons"> Villain and enters the HQ.`);
        ...
        await dailyBugleSelectHighAttackVillain(highestAttackVillains, selectedHQIndex);
    } else {
        onscreenConsole.log("No Villains available to enter HQ.");
    }
}

async function dailyBugleSelectHighAttackVillain(highestAttackVillains, selectedHQIndex) {
    updateGameBoard();
    return new Promise((resolve) => {
        const popup = document.querySelector('.card-choice-city-hq-popup');
        const modalOverlay = document.getElementById('modal-overlay');
        ...
        // Set popup content
        titleElement.textContent = 'SCHEME TWIST';
        instructionsElement.innerHTML = 'There are multiple Villains with the highest <img src="Visual Assets/Icons/Attack.svg" alt="Attack Icon" class="console-card-icons">. Select one to enter the HQ.';
        ...
        const availableVillains = highestAttackVillains.filter(villain => 
            city.includes(villain) // Make sure villain is still in city
        );
        ...
        confirmButton.onclick = async (e) => {
            ...
            onscreenConsole.log(`<span class="console-highlights">${selectedVillain.name}</span> enters the HQ.`);
...
async function weaveAWebOfLiesBystanderRescue() {
     return new Promise((resolve) => {
        ...
        if (totalRecruitPoints === 0) {
            onscreenConsole.log(`Weave a Web of Lies: Not enough <img src='Visual Assets/Icons/Recruit.svg' alt='Recruit Icon' class='console-card-icons'> points to rescue a Bystander.`)
        ...
        setTimeout(async () => {
            ...
            // Update title
            document.querySelector('.info-or-choice-popup-title').innerHTML = 'WEAVE A WEB OF LIES';
            ...
            if (previewArea) {
                previewArea.style.backgroundImage = "url('Visual Assets/Schemes/PtTR_weaveAWebOfLies.webp')";
            ...
            confirmButton.onclick = async () => {
                onscreenConsole.log(`Weave a Web of Lies: 1 <img src="Visual Assets/Icons/Recruit.svg" alt="Recruit Icon" class="console-card-icons"> paid to rescue a Bystander.`);
            ...
            denyButton.onclick = () => {
                onscreenConsole.log(`Weave a Web of Lies: You have chosen not to rescue a Bystander.`);
                closeInfoChoicePopup();
                resolve(); // Fixed: added parentheses
            };
        }, 10);
...
async function theCloneSagaTwist() {
    ...
    if (cardsYouHave.filter(item => item.type === "Hero" && item.color !== "Grey").filter((item, index, arr) => arr.findIndex(i => i.name === item.name) !== index).length < 2) {
    onscreenConsole.log(`You are unable to reveal two non-grey Heroes with the same card name.`);
    await theCloneSagaDiscard();
        ...
        return new Promise((resolve) => {
            setTimeout(() => {   
                ...
                const previewArea = document.querySelector('.info-or-choice-popup-preview');
                if (previewArea) {
                    previewArea.style.backgroundImage = "url('Visual Assets/Schemes/PtTR_theCloneSaga.webp')";
                    previewArea.style.backgroundSize = 'contain';
                    previewArea.style.backgroundRepeat = 'no-repeat';
                    previewArea.style.backgroundPosition = 'center';
                    previewArea.style.display = 'block';
                ...
                confirmButton.onclick = () => {
                    onscreenConsole.log(`You are able to reveal two non-grey Heroes with the same card name and have escaped discarding!`);
                    cleanup();
                };

                denyButton.onclick = async () => {
                    onscreenConsole.log(`You have chosen not to reveal two non-grey Heroes with the same card name.`);
                    cleanup();
                    await  theCloneSagaDiscard();
                };
            }, 10);
...
async function theCloneSagaDiscard() {
   updateGameBoard();
    return new Promise((resolve) => {
        ...
        // Set popup content
        titleElement.textContent = 'Scheme Twist';
        instructionsElement.innerHTML = 'Select cards to discard until you have 3 cards remaining.';
        ...
        if (cardsToDiscard <= 0) {
            onscreenConsole.log('You already have 3 or fewer cards in hand. No cards to discard.');
            resolve();
            return;
        }
        ...
        confirmButton.onclick = async (e) => {
            ...
            setTimeout(async () => {
                ...
                if (discardedCardNames.length > 0) {
                    ...
                    onscreenConsole.log(`Discarded ${formattedNames} to satisfy Scheme Twist.`);
                }
...
async function wallCrawlRecruit(card) {
    return new Promise(async (resolve) => {
        ...
        confirmButton.onclick = async () => {
            ...
            card.revealed = true;
            onscreenConsole.log(`<span class="console-highlights">${card.name}</span> has been put on top of your deck.`);
        ...
        noThanksButton.onclick = () => {
            ...
            onscreenConsole.log(`<span class="console-highlights">${card.name}</span> has been added to your discard pile.`);
            closeDrawChoicePopup(wasModalOverlayVisible);
...
async function carrionFeast() {
  if (playerDeck.length === 0) {
        if (playerDiscardPile.length > 0) {
          ...
          onscreenConsole.log(`No cards available for <span class="console-highlights">Carrion</span> to feast upon.`);
        return;
        }
        }
  ...
  playerDeck.pop();
  koPile.push(topCard);
  onscreenConsole.log(`<span class="console-highlights">Carrion</span> feasted upon <span class="console-highlights">${topCard.name}</span>, KOing them.`);
  koBonuses();
    updateGameBoard();
...
async function demogoblinFeast() {
  if (playerDeck.length === 0) {
        if (playerDiscardPile.length > 0) {
          ...
          onscreenConsole.log(`No cards available for <span class="console-highlights">Demogoblin</span> to feast upon.`);
        return;
        }
        }
  const topCard = playerDeck[playerDeck.length - 1];
  ...
  playerDeck.pop();
  koPile.push(topCard);
  onscreenConsole.log(`<span class="console-highlights">Demogoblin</span> feasted upon <span class="console-highlights">${topCard.name}</span>, KOing them.`);
  koBonuses();
    updateGameBoard();
}
...
async function doppelgangerFeast() {
  if (playerDeck.length === 0) {
        if (playerDiscardPile.length > 0) {
          ...
          onscreenConsole.log(`No cards available for <span class="console-highlights">Doppelganger</span> to feast upon.`);
        return;
        }
        }
  const topCard = playerDeck[playerDeck.length - 1];
  ...
  playerDeck.pop();
  koPile.push(topCard);
  onscreenConsole.log(`<span class="console-highlights">Doppelganger</span> feasted upon <span class="console-highlights">${topCard.name}</span>, KOing them.`);
  koBonuses();
    updateGameBoard();
}
...
async function shriekFeast() {
  ...
  if (topCard.cost === 0) {
    onscreenConsole.log(`<span class="console-highlights">${topCard.name}</span> cost 0, so <span class="console-highlights">Shriek</span> forces you to gain a Wound!`);
    await drawWound();
    } else {
      onscreenConsole.log(`Thankfully, <span class="console-highlights">${topCard.name}</span> did not cost 0 and you have escaped gaining a Wound.`);
      }
...
async function shriekEscape() {
    onscreenConsole.log(`Escape! <span class="console-highlights">Shriek</span> escapes, forcing you to gain a Wound!`);
    await drawWound();
  updateGameBoard();
}
...
async function chameleonFight(card) {
    return new Promise((resolve) => {
        ...
        if (cardCopy.unconditionalAbility && cardCopy.unconditionalAbility !== "None") {
            ...
            if (typeof abilityFunction === 'function') {
                ...
                console.error(`Unconditional ability function ${cardCopy.unconditionalAbility} not found`);
            }
        }

        // Handle conditional ability
        abilityPromise.then(() => {
            if (cardCopy.conditionalAbility && cardCopy.conditionalAbility !== "None") {
                const { conditionType, condition } = cardCopy;
                if (isConditionMet(conditionType, condition)) {
                    if (autoSuperpowers) {
                        const conditionalAbilityFunction = window[cardCopy.conditionalAbility];
                        if (typeof conditionalAbilityFunction === 'function') {
                            ...
                            console.error(`Conditional ability function ${cardCopy.conditionalAbility} not found`);
                        ...
                        return new Promise((resolveConditional, rejectConditional) => {
                            setTimeout(() => {
                                ...
                                denyButton.onclick = () => {
                                    onscreenConsole.log(`You have chosen not to activate <span class="console-highlights">${cardCopy.name}</span><span class="bold-spans">'s</span> ability.`);
                                    cleanup();
                                };
                            }, 10);
                        });
...
async function kravenTheHunterEscape() {
  ...
  try {
    onscreenConsole.log(`Escape! <span class="console-highlights">Kraven the Hunter</span> also KOs the highest cost Hero in the HQ.`);
...
function koHeroKraven(hero) {
    ...
    onscreenConsole.log(`<span class="console-highlights">Kraven the Hunter</span> also KO'd <span class="console-highlights">${hero.name}</span>.`);
...
function koHero(hero) {
    ...
    onscreenConsole.log(`<span class="console-highlights">Carrion</span> feasted upon <span class="console-highlights">${hero.name}</span>, KOing them.`);
...
function showHeroSelectionPopupKO(heroes, onHeroSelected) {
    ...
    return new Promise((resolve) => {
        ...
        instructionsElement.innerHTML = 'There are multiple heroes with the same cost to KO. Select one.';
...
onscreenConsole.log(`Escape! <span class="console-highlights">Sandman</span> requires you to reveal an <img src="Visual Assets/Icons/Instinct.svg" alt="Instinct Icon" class="console-card-icons"> Hero or gain a Wound!`);
...
if (cardsYouHave.filter(item => item.classes && item.classes.includes('Instinct')).length === 0) {
        onscreenConsole.log(`You are unable to reveal an <img src="Visual Assets/Icons/Instinct.svg" alt="Instinct Icon" class="console-card-icons"> Hero.`);
        drawWound();
    } else {
        return new Promise((resolve) => {
            setTimeout(() => {   
                ...
                // Update title
                document.querySelector('.info-or-choice-popup-title').innerHTML = 'SANDMAN';
                ...
                const previewArea = document.querySelector('.info-or-choice-popup-preview');
                if (previewArea) {
                    previewArea.style.backgroundImage = "url('Visual Assets/Villains/PtTR_SinisterSix_Sandman.webp')";
                    previewArea.style.backgroundSize = 'contain';
                    previewArea.style.backgroundRepeat = 'no-repeat';
                    previewArea.style.backgroundPosition = 'center';
                    previewArea.style.display = 'block';
                ...
                confirmButton.onclick = () => {
                    onscreenConsole.log(`You are able to reveal an <img src="Visual Assets/Icons/Instinct.svg" alt="Instinct Icon" class="console-card-icons"> Hero and have escaped gaining a Wound!`);
                    cleanup();
                };

                denyButton.onclick = () => {
                    onscreenConsole.log(`You have chosen not to reveal an <img src="Visual Assets/Icons/Instinct.svg" alt="Instinct Icon" class="console-card-icons"> Hero.`);
                    drawWound();
                    cleanup();
                };
            }, 10);
...
onscreenConsole.log(`Escape! <span class="console-highlights">Vulture</span> requires you to reveal an <img src="Visual Assets/Icons/Instinct.svg" alt="Instinct Icon" class="console-card-icons"> Hero or gain a Wound!`);
...
if (cardsYouHave.filter(item => item.classes && item.classes.includes('Instinct')).length === 0) {
        ...
        return new Promise((resolve) => {
            setTimeout(() => {   
                ...
                // Update title
                document.querySelector('.info-or-choice-popup-title').innerHTML = 'VULTURE';
                ...
                const previewArea = document.querySelector('.info-or-choice-popup-preview');
                if (previewArea) {
                    previewArea.style.backgroundImage = "url('Visual Assets/Villains/Vulture.webp')";
                    previewArea.style.backgroundSize = 'contain';
                    previewArea.style.backgroundRepeat = 'no-repeat';
                    previewArea.style.backgroundPosition = 'center';
                    previewArea.style.display = 'block';
...
onscreenConsole.log(`Ambush! <span class="console-highlights">Shocker</span> forces you to reveal an <img src="Visual Assets/Icons/Instinct.svg" alt="Instinct Icon" class="console-card-icons"> Hero or discard!`);
...
if (cardsYouHave.filter(item => item.classes && item.classes.includes('Instinct')).length === 0) {
        ...
        return new Promise((resolve) => {
            setTimeout(() => {   
                ...
                // Update title
                document.querySelector('.info-or-choice-popup-title').innerHTML = 'SHOCKER';
                ...
                const previewArea = document.querySelector('.info-or-choice-popup-preview');
                if (previewArea) {
                    previewArea.style.backgroundImage = "url('Visual Assets/Villains/PtTR_SinisterSix_Shocker.webp')";
                    previewArea.style.backgroundSize = 'contain';
                    previewArea.style.backgroundRepeat = 'no-repeat';
                    previewArea.style.backgroundPosition = 'center';
                    previewArea.style.display = 'block';
                ...
                confirmButton.onclick = () => {
                    onscreenConsole.log(`You are able to reveal an <img src="Visual Assets/Icons/Instinct.svg" alt="Instinct Icon" class="console-card-icons"> Hero and have escaped discarding!`);
                    cleanup();
                };
...
async function vultureAmbush() {
    onscreenConsole.log(`Ambush! <span class="console-highlights">Vulture</span> may move deeper into the city.`);
    ...
    if (!villainAtZero && !villainAtTwo) {
        onscreenConsole.log(`No villains on the Rooftops or Bridge to swap <span class="console-highlights">Vulture</span> with.`);
    ...
    if ((villainAtZero && !villainAtTwo) || (!villainAtZero && villainAtTwo)) {
        ...
        // Perform the swap
        if (villainAtFour) {
            onscreenConsole.log(`<span class="console-highlights">${city[sourceIndex].name}</span> from ${sourceName} swapped places with <span class="console-highlights">${city[4].name}</span>.`);
        } else {
            onscreenConsole.log(`<span class="console-highlights">${city[sourceIndex].name}</span> moved from ${sourceName} to the Sewers.`);
        }
    ...
    return new Promise((resolve) => {
        ...
        noThanksButton.style.display = 'none';
        document.getElementById('villain-movement-context').innerHTML = 'Swap <span class="console-highlights">Vulture</span> with the Villain on the Rootops or Bridge.';
        ...
        function selectCell(cellElement) {
            ...
            const cellKey = Object.keys(villainCells).find(key => villainCells[key] === cellElement);
            ...
            // Draw arrow from selected cell to bank
            drawArrow(selectedCell, villainCells.bank);
        ...
        confirmButton.onclick = () => {
            if (selectedCell) {
                const cellKey = Object.keys(villainCells).find(key => villainCells[key] === selectedCell);
                const sourceIndex = cellKey === 'bridge' ? 0 : 2;
                const sourceName = cellKey === 'bridge' ? "Bridge" : "Rooftops";
                
                // Perform the swap
                if (city[4]) {
                    onscreenConsole.log(`<span class="console-highlights">${city[sourceIndex].name}</span> from ${sourceName} swapped with <span class="console-highlights">${city[4].name}</span>.`);
                } else {
                    onscreenConsole.log(`<span class="console-highlights">${city[sourceIndex].name}</span> moved.`);
                }
...
function moonKnightLunarCommunion() {
  onscreenConsole.log(`Whenever you defeat a Villain on the Rooftops this turn, you may KO one of your cards or a card from your discard pile.`);

moonKnightLunarCommunionKO = true;
updateGameBoard();
}
...
function moonKnightLunarCommunionKOChoice() {
    return new Promise((resolve) => {
        ...
        instructionsElement.innerHTML = `You may KO one of your cards or a card from your discard pile.`;
        ...
        const playedCards = combinedCards.filter(item => item.source === 'played');
        ...
        function createCardElement(cardItem, location, source, row) {
            const cardElement = document.createElement('div');
            cardElement.className = 'popup-card';
            cardElement.setAttribute('data-card-id', cardItem.id || cardItem.card.id);
            cardElement.setAttribute('data-location', location);
            cardElement.setAttribute('data-source', source);
            
            // Create card image
            const cardImage = document.createElement('img');
            cardImage.src = cardItem.image || cardItem.card.image;
            cardImage.alt = cardItem.name || cardItem.card.name;
            cardImage.className = 'popup-card-image';
            ...
            const handleHover = () => {
                ...
                previewElement.innerHTML = '';
                const previewImage = document.createElement('img');
                previewImage.src = cardItem.image || cardItem.card.image;
                previewImage.alt = cardItem.name || cardItem.card.name;
                previewImage.className = 'popup-card-preview-image';
                previewElement.appendChild(previewImage);
            ...
            cardElement.addEventListener('click', (e) => {
                ...
                const cardData = location === 'discard' ? cardItem : cardItem.card;
                const cardName = cardItem.name || cardItem.card.name;

                if (selectedCard === cardData && selectedLocation === location) {
                    // Deselect
                    selectedCard = null;
        ...
        confirmButton.onclick = (e) => {
            ...
            setTimeout(() => {
                ...
                if (selectedLocation === 'your-cards') {
                ...
                if (koIndex !== -1 || selectedSource === 'played') {
...
if (playerDeck.length === 0) {
 ...
 if ((topCardPlayerDeck.classes && (topCardPlayerDeck.classes.includes("Instinct") || topCardPlayerDeck.classes.includes("Tech")))) {
    playSFX('card-draw');
...
onscreenConsole.log(`Whenever you defeat a Villain on the Rooftops this turn, rescue Bystanders equal to that Villain's printed Victory Points.`);
...
function moonKnightGoldenAnkhOfKhonshuBystanderCalculation(villainCard) {
    return new Promise(async (resolve) => {
        ...
        if (numberToRescue.length === 0) {
            onscreenConsole.log('The defeated Villain has no Victory Points. No Bystanders rescued.');
            resolve(false);
            return;
        }
        ...
        if (numberToRescue > bystanderCount) {
            onscreenConsole.log(`<span class="console-highlights">${villainCard.name}</span> has ${numberToRescue} Victory Point${numberToRescue === 1 ? '' : 's'}; however, you do not have that many Bystanders remaining. All that remain will be rescued.`);
            ...
            onscreenConsole.log(`<span class="console-highlights">${villainCard.name}</span> has ${numberToRescue} Victory Points. Rescuing ${numberToRescue === 1 ? 'a Bystander' : 'that many Bystanders'} now.`);
...
if (isCityEmpty()) {
    ...
    const overlay = document.getElementById('modal-overlay');
    const noThanksButton = document.getElementById('no-thanks-villain-movement');
    document.getElementById('villain-movement-context').innerHTML = 'You may move a Villain to the Rooftops. If another Villain is already there, swap them.';
    const confirmButton = document.getElementById('confirm-villain-movement');
    const selectionArrow = document.getElementById('selection-arrow');
    ...
    function selectCell(cellElement) {
        ...
        if (hasVillain && selectedCells.length === 0) {
            ...
            if (firstSelectedIndex === rooftopsIndex) {
                onscreenConsole.log(`The selected Villain is already at the Rooftops. Please select a different Villain.`);
                cellElement.classList.remove('selected');
                selectedCells = [];
                firstSelectedIndex = null;
                return;
    ...
    return new Promise((resolve) => {
        ...
        confirmButton.onclick = async () => {
            if (selectedCells.length === 1) {
                ...
                const isRooftopsEmpty = rooftopsImage && rooftopsImage.src.includes('BlankCardSpace.webp');

                if (isRooftopsEmpty) {
                    // Move the villain to the empty Rooftops
                    console.log("Moving villain to empty Rooftops");
                    onscreenConsole.log(`<span class="console-highlights">${city[firstIndex].name}</span> moved to the Rooftops.`);
                    ...
                    onscreenConsole.log(`<span class="console-highlights">${city[firstIndex].name}</span> swapped places with <span class="console-highlights">${city[rooftopsIndex].name}</span> at the Rooftops.`);
...
function scarletSpiderStingOfTheSpiderDrawChoice(card) {
    return new Promise(async (resolve) => {
        ...
        if (!cardInDeck) {
            console.warn(`Sting of the Spider: Card ${card.name} not found in player deck`);
            resolve();
            return;
        }
        ...
        confirmButton.onclick = async () => {
            ...
            if (deckIndex !== -1) {
                ...
                // Log the draw
                onscreenConsole.log(`Drew <span class="console-highlights">${card.name}</span> from deck.`);
                updateGameBoard();
            }
...
async function spiderWomanRadioactiveSpider() {
    return new Promise((resolve) => {
        ...
        if (playerHand.length === 0) {
            ...
            updateGameBoard();
            onscreenConsole.log("No cards in Hand to put on your deck. You are unable to play this card.");
            resolve(false);
            return;
        }
        ...
        instructionsElement.innerHTML = 'Select a card to put on top of your deck in order to play <span class="console-highlights">Spider-Woman - Radioactive Spider</span>.';
    ...
    document.querySelector('.card-choice-popup-selectionrow1-container').style.top = '28%';
        ...
        confirmButton.onclick = async (e) => {
            ...
            if (index !== -1) {
                ...
                onscreenConsole.log(`You have put <span class="console-highlights">${chosenCard.name}</span> on top of your deck, allowing you to play <span class="console-highlights">Spider-Woman - Radioactive Spider</span>.`);
        ...
        noThanksButton.onclick = (e) => {
            ...
            onscreenConsole.log(`You have chosen not to put a card on top of your deck, preventing you from playing <span class="console-highlights">Spider-Woman - Radioactive Spider</span>.`);
...
if (!isEligible) {
  ...
  // ... keep your existing click/hover handlers here ...
        ...
        const eligibleHeroesForRecruit = hq.filter((hero, index) => {
            const explosionValue = explosionValues[index] || 0;
            return hero && 
                   explosionValue < 6; // Not destroyed (removed team requirement)
        });
        ...
        confirmButton.onclick = async (e) => {
            ...
            const spiderFriends = cardsYouHave.filter(item => item.team === "Spider Friends");
            ...
            if (spiderFriends.length > 0) {
                ...
                if (recruitedCardIndex !== -1) {
                    const [recruitedHero] = playerDiscardPile.splice(recruitedCardIndex, 1);
                    playerDeck.push(recruitedHero);
                    recruitedHero.revealed = true;
                    onscreenConsole.log(`<img src="Visual Assets/Icons/Spider Friends.svg" alt="Spider Friends Icon" class="console-card-icons"> Hero played. Superpower Ability activated. <span class="console-highlights">${recruitedHero.name}</span> has been put on top of your deck.`);
...
function symbioteSpiderManThwip() {
    return new Promise((resolve) => {
        ...
        if (playerHand.length < 2) {
            console.log("Not enough cards in Hand. You need at least 2 cards to play this card.");
            const unplayedCard = cardsPlayedThisTurn[cardsPlayedThisTurn.length - 1];
            playerHand.push(unplayedCard);
            cardsPlayedThisTurn.pop(unplayedCard);
            totalAttackPoints -= unplayedCard.attack;
            ...
            cumulativeRecruitPoints -= unplayedCard.recruit;
            updateGameBoard();
            onscreenConsole.log("Not enough cards in Hand to put on your deck. You need at least 2 cards to play this card.");
            resolve(false);
            return;
        }
        ...
        instructionsElement.innerHTML = 'Select 2 cards to put on top of your deck in order to play <span class="console-highlights">Symbiote Spider-Man - Thwip!</span>. The first card selected will be placed on your deck first, then the second card on top.';
        ...
        sortedHand.forEach((card) => {
            ...
            cardElement.addEventListener('click', (e) => {
                ...
                if (cardIndex !== -1) {
                    ...
                    // Update instructions and confirm button
                    updateInstructions();
                    document.getElementById('card-choice-popup-confirm').disabled = selectedCards.length !== 2;
                } else {
...
confirmButton.onclick = async (e) => {
    ...
    onscreenConsole.log(`You have put <span class="console-highlights">${selectedCards[0].name}</span> and <span class="console-highlights">${selectedCards[1].name}</span> on top of your deck (in that order), allowing you to play <span class="console-highlights">Symbiote Spider-Man - Thwip!</span>.`);
    ...
    resolve(true);
        ...
        noThanksButton.onclick = (e) => {
            ...
            onscreenConsole.log(`You have chosen not to put 2 cards on top of your deck, preventing you from playing <span class="console-highlights">Symbiote Spider-Man - Thwip!</span>.`);
...
if (playerDeck.length === 0) {
  ...
  playerDeck.pop();
  koPile.push(topCardPlayerDeck);
  onscreenConsole.log(`You are the only player to choose. <span class="console-highlights">Carnage</span> feasts on the top card of your deck. <span class="console-highlights">${topCardPlayerDeck.name}</span> has been KO'd.`);
  koBonuses();
  updateGameBoard();
}
...
function chooseBystanderKOFromVPSingle() {
    ...
    return new Promise((resolve) => {
        ...
        instructionsElement.innerHTML = 'Select one Bystander to KO from your Victory Pile.';
        ...
        bystandersInVP.forEach((card) => {
            ...
            cardElement.addEventListener('click', (e) => {
                ...
                if (selectedBystander && selectedBystander.id === card.id) {
                    ...
                    if (selectedBystander) {
                        const previousSelectedElement = document.querySelector(`[data-card-id="${selectedBystander.id}"] img`);
        ...
        confirmButton.onclick = (e) => {
            ...
            setTimeout(() => {
                ...
                onscreenConsole.log(`<span class="console-highlights">${selectedBystander.name}</span> KO'd from Victory Pile.`);
                koBonuses();
...
function mysterioMasterOfIllusions() {
    ...
    const mysterioTactics = victoryPile.filter(card => card.name === "Mysterio Mastermind Tactic");
